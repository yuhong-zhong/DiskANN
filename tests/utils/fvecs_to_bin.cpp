...
//Convert byte types
void block_convert(std::ifstream& reader, std::ofstream& writer, _u8* read_buf,
...
int main(int argc, char** argv) {
  if (argc < 3 || argc > 4) {
    std::cout << argv[0] << " input_vecs output_bin (optional)datatype<int8/uint8/float>" << std::endl;
    exit(-1);
  }

  // 0:float, 1:uint8, 2:int8
  int convertType = 0; 
  ...
  for (_u64 i = 0; i < nblks; i++) {
    ...
    if (convertType == 0) {
      block_convert(reader, writer, (float*) read_buf, (float*) write_buf,
                    cblk_size, ndims);
    } else {
