...
void delete_points(diskann::AbstractIndex &index, diskann::IndexWriteParameters &delete_params)
{
    try
    {
        std::ifstream points = std::ifstream {"points_to_delete.bin"};
        size_t i;
        while (points >> i) {
            index.lazy_delete(static_cast<TagT>(i));
        }
        // std::cout << std::endl
                        ...
                        const std::vector<std::string> &query_filters, const float fail_if_recall_below)
{
    ...
    for (uint32_t test_id = 0; test_id < Lvec.size(); test_id++)
    {
        ...
        int64_t num_batches = (int64_t)(query_num / 10) + (query_num % 100 == 0 ? 0: 1);

        for (int64_t batch = 0; batch < num_batches; batch++){
            int64_t start = batch * 100;
            int64_t end = (batch + 1) * 100;
            ...
            diskann::IndexWriteParameters delete_params = diskann::IndexWriteParametersBuilder(100, 64)
                                                    .with_max_occlusion_size(500)
                                                    .with_alpha(1.2)
                                                    .with_num_threads(num_threads)
                                                    .with_filter_list_size(10)
                                                    .build();
            delete_points<T, TagT>(*index, delete_params);
        }
