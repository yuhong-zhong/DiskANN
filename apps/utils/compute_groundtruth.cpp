             ...
             const diskann::Metric &metric, const std::string &tags_file = std::string(""), uint64_t start_offset = 0, uint64_t end_offset = 0)
{
    ...
    for (size_t i = 0; i < nqueries; i++)
    {
        ...
        for (auto iter : cur_res)
        {
            ...
            {
                closest_points[i * k + j] = (int32_t)iter.first + start_offset + 1;
            }
...
int main(int argc, char **argv)
{
    ...
    uint64_t K;
    uint64_t start_offset, end_offset, total_points;
    ...
    try
    {
        ...
        //desc.add_options()("total_points", po::value<uint64_t>(&total_points)->required(), "total_points");
        
        // if (end_offset == 0){
        //     end_offset = total_points;
        // }
